...
const ProductCard = ({ item, allResults }) => {
  ...
  // Logic for Price Matrix (Matching products by name)
  ...
  // Rating & Savings Logic
  const dealScore = computeDealScore(item.current_price, item.original_price);
  const rating = item.rating_score > 0 ? Number(item.rating_score) : null;
  const savings = item.original_price && item.original_price > item.current_price
    ? item.original_price - item.current_price
    : null;

  const nameLower = item.name?.toLowerCase() || '';
  const isJV = nameLower.includes('jv');
  const isCPID = nameLower.includes('cpid');

  return (
    <article className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden flex flex-col group transition-all hover:shadow-md">

      {/* Image Section */}
      <div className="relative bg-gray-50 h-48 flex items-center justify-center p-4 overflow-hidden">
        {dealScore !== null && (
          <div className={`absolute top-3 left-3 z-10 ${getDealScoreColor(dealScore)} text-[10px] font-black px-2 py-1 rounded-full flex items-center gap-1 shadow-sm`}>
            <span>⚡ {dealScore}% OFF</span>
          </div>
        )}

        <div className="absolute top-3 right-3 z-10 bg-white/90 backdrop-blur-sm border border-gray-100 text-[10px] font-bold px-2 py-1 rounded-full text-gray-600 flex items-center gap-1">
          <span>{getPlatformEmoji(item.store)}</span>
          <span>{getPlatformLabel(item.store)}</span>
        </div>

        <img
          src={item.image_url || `https://placehold.co/200x180?text=No+Image`}
          alt={item.name}
      ...
      <div className="flex flex-col flex-1 p-4 gap-3">
        ...
        <div>
          ...
          {(isJV || isCPID) && (
            <div className="mt-1 flex gap-1">
              {isJV && <span className="bg-gray-800 text-white text-[9px] font-bold px-1.5 py-0.5 rounded uppercase">JV</span>}
              {isCPID && <span className="bg-blue-600 text-white text-[9px] font-bold px-1.5 py-0.5 rounded uppercase">CPID</span>}
            </div>
          )}
        </div>
        ...
        <button
          ...
          className="text-[11px] text-orange-500 hover:text-orange-600 font-bold text-left transition-colors flex items-center gap-1"
        >
        ...
        <a
          ...
          target="_blank"
          rel="noopener noreferrer"
          className="mt-auto block text-center bg-gray-900 hover:bg-black text-white font-bold py-3 rounded-xl transition-all active:scale-95 text-xs uppercase tracking-wider"
        >
          View on {getPlatformLabel(item.store)}
        </a>
      </div>
    </article>
  );
...
//   const rawRating = item['rating-score'] || item.rating_score || item.rating;
//   const rating = rawRating > 0 ? Number(rawRating) : null;
...
//           <span>{getPlatformEmoji(item.store)}</span>
//           <span>{getPlatformLabel(item.store)}</span>
//           {/* <span>value in item.store is:{item.store}</span> */}
//         </div>
...
//             );
//           })}
//           <span className='text-xs font-bold text-gray-700'>Sold:{item.item_sold_count}</span>
//         </div>
...
//                   <span
//                     key={star}
//                     className={`text-sm leading-none ${star <= Math.round(rating) ? 'text-amber-400' : 'text-gray-300'
//                       }`}
//                   >
//                     ★
//                   </span>
